...
public class HouseController {
    ...
    public Result selectPageAvailable(@RequestParam("pageNum") Integer pageNum,
        ...
        PageHelper.startPage(pageNum,pageSize);
        //查询数据库，需要用到like查询
        List<House> list = houseService.list(new QueryWrapper<House>().like("title", title).eq("status","0").eq("del", "0").orderByDesc("id"));
        //封装一下查询的结果集
        PageInfo<House> pageInfo = PageInfo.of(list);
        return Result.success(pageInfo);
    }
     ...
     *  更新房源状态
     */
    @ApiOperation(value = "更新房源状态", notes = "更新房源状态")
    @PutMapping("updateStatus")
    public Result updateStatus(@RequestBody House house){
        boolean b = houseService.updateById(house);
        if(b){
            return Result.success();
        }else{
